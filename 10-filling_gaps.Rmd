...
```{r Loading the station list and creating a table, echo=FALSE, warning=FALSE, message=FALSE}
...
positions_in_station_list<-c(2,4,3,6,5)
patch_weather<-list()
...
This function uses the previously downloaded data to patch the main data. It is possible to exclude unsuitable data, but in my case I didnt apply any restrictions.
```{r Patching the main dataset with the newly downloaded data, eval=TRUE}
...
The tables show the patching progress between the application of the five different datasets on the main data. We can see that the number of `gaps_remain` decreases, but also that the `mean_bias` and `stdev_bias` increase with more distant and unsuitable stations.
After the third station 19 `gaps_remain` and also the two subsequent stations are not able to patch those gaps. I somehow thought that those gaps all belong to my artificial gap, but after checking the data, I saw that they are distributed over all years.
I used this method as an example here, usually data with a `stdev_bias` that is too high would not benefit the main data.
\
...
```{r, echo=FALSE}
title <- paste("Patch statistics for",names(patched$statistics)[4])
patched$statistics[[4]] %>% 
kbl(escape = F, format = "html", caption = title) %>%
  kable_paper("hover", full_width = T, position = "left")
```
...
```{r, echo=FALSE}
title <- paste("Patch statistics for",names(patched$statistics)[5])
patched$statistics[[5]] %>% 
kbl(escape = F, format = "html", caption = title) %>%
  kable_paper("hover", full_width = T, position = "left")
```
\
...
final_patched_fixed_weather$QC %>% 
kbl(escape = F, format = "html", caption = title) %>%
  kable_paper("hover", full_width = T, position = "left")
```
\
...
I fixed the last single gaps with the `fixed_weather` command, which interpolates missing days. For short gaps, this should not lead to problems. For further analyses, i will use the original dataset without the artificial gap. Gaps of single days are interpolated with `fixed_weather`.

```{r Loading original data and fixing single missing days, eval=FALSE, echo=FALSE}
cleaned_weather <- read.csv("data/CapeTown_chillR_weather.csv")
```
