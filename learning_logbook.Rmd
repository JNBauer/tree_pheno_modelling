...
## Lesson 08: Making hourly temperatures
...
weather <- cleaned_weather$`CAPE TOWN INTL`$weather
kable(cleaned_weather$weather[1:20,]) %>%
  kable_styling("striped", position = "left", font_size = 8)

print(cleaned_weather$`CAPE TOWN INTL`$database)
```
...
write.csv(cleaned_weather$weather,"data/CapeTown_chillR_weather.csv",row.names=FALSE)
```
...
require(dplyr)
require(kableExtra)
CapeTown_weather <- read.csv("data/CapeTown_chillR_weather.csv")

fixed_weather <- fix_weather(CapeTown_weather)


CapeTown_weather_gap <- CapeTown_weather[-c(4231:4444),]

fixed_weather <- fix_weather(CapeTown_weather_gap)



kbl(fixed_weather$QC[1:30,], escape = F) %>%
  kable_paper("hover", full_width = F, position = "left")
...
```{r, eval=FALSE}
positions_in_station_list<-c(1,10)
...
patched_fixed_weather2 <- fix_weather(patched_fixed_weather)
patched_fixed_weather2$QC
...
require(chillR)
fixed_weather
patched_fixed_weather <- fixed_weather$weather

head(patched_fixed_weather)

temp<-temperature_generation(patched_fixed_weather,
                             years=c(1990,2005),
                             sim_years = c(2001,2100))


temperatures<-cbind(patched_fixed_weather[
  which(patched_fixed_weather$Year %in% 1998:2005),] ,Data_source="observed")
...
chill_observed<-chilling(
  ...
  Start_JDay = 121, # Beginning May
  End_JDay = 243) # End of August
...
colnames(chill_comparison) == colnames(chill_simulated)
#chill_comparison <- chill_comparison[-"Interpolated_days"]

chill_comparison <- rbind(chill_comparison, chill_simulated)
...
FH <- function(hourtemps)
  {
  hourtemps[,"Freezing_hours"] <- hourtemps$Temp<0
  return(hourtemps)
}
hourtemps_plus_FH <- FH(hourtemps)
...
models<-list(Chilling_Hours=Chilling_Hours,Utah_Chill_Units=Utah_Model,Chill_Portions=
  Dynamic_Model,GDH=GDH,Freezing_hours=FH)
...
freeze_observed <- tempResponse(hourtemps[[1]],
                     Start_JDay = 275, End_JDay = 305, 
                     models=list(Freezing_hours=FH),
                     whole_record = TRUE)

freeze_observed <- tempResponse(
                    stack_hourly_temps(
                     temperatures[which(temperatures$Data_source=="observed"),]),
                     Start_JDay = 275, End_JDay = 305, 
                     models=list(Freezing_hours=FH),
